...
export function App() {
  ...
  return <div className={`app-shell ${sidebarCollapsed ? "sidebar-collapsed" : ""} ${sidebarOpen ? "mobile-sidebar-open" : ""}`}>
    ...
    <SessionTabs sessions={workspace.allSessions} selectedID={workspace.selectedID} hasMore={Boolean(workspace.sessions.hasNextPage)} loadingMore={workspace.sessions.isFetchingNextPage} creating={workspace.create.isPending} onSelect={(id) => { workspace.select(id); setSidebarOpen(false); }} onCreate={() => { workspace.create.mutate(); setSidebarOpen(false); }} onLoadMore={() => workspace.sessions.fetchNextPage()} />
